...
function fileExists(bucketName, filename) {
    return new Promise(resolve => {
        storage.bucket(bucketName).file(filename).getMetadata()
            .then(results => {
                return resolve(true);
            })
            .catch(err => {
...
router.get('/reupload', (req, res, next) => {
    mode = 'reupload';
    currentResult = res;
    currentRequest = req;

    if (logged_in_user != null) {
        res.render('reupload', { title: 'Reupload', email: logged_in_user.email });
    } else {
        res.redirect('/');
    }
});
...
// Requires logged in.
router.get('/upload/:key', function(req, res, next) {
    currentResult = res;
    currentRequest = req;
    if (logged_in_user != null) {
        var sendFile = null;
        var newFileName = "";
        database.ref('clientFiles').child(req.params.key).once('value')
            .then(fileSnap => {
                var gotFile = fileSnap.val()[Object.keys(fileSnap.val())[0]];
                sendFile = Object.keys(fileSnap.val())[0];
                var dir = gotFile.email;
                newFileName = sendFile;
                res.render('upload', { title: 'Upload', email: logged_in_user.email, filename: newFileName, reupload: 'true' });
            });
...
router.get('/upload', function(req, res, next) {
    ...
    currentRequest = req;
    if (logged_in_user != null) {
        res.render('upload', { title: 'Upload', email: logged_in_user.email, filename: '', reupload: 'false' });
    } else {
        res.redirect('/');
    }
});
...
router.post("/api/upload", function(req, res) {
    ...
    upload(req, res, function(err) {
        ...
        if (req.body.paper_name == "") {
            return res.end("Enter paper name");
        }
        ...
        fileExists(bucketName, "uploads/" + req.body.email + "/" + newFileName + "/.folderExists").
        then(exists => {
            if (exists && req.body.reupload == "false") {
                return res.end("Paper with the same name already exists");
            } else {
                var cloudPromises = [];
                var localPromises = [];
                var fileNames = [];
                ...
                var cloudInit = bucket.upload("keys/welcome.txt", { destination: "uploads/" + req.body.email + "/" + newFileName + "/.folderExists" }, (err, file) => {});


                cloudPromises.push(cloudInit);
                ...
                for (var i = 2; i >= 0; i--) {
                    ...
                    // cloudPromises.push(localUpload);
                    cloudPromises.push(cloudUpload);

                }
                cloudPromises.push(mkdirp("./uploads/" + dir + "/" + newFileName, err => {}));
                ...
                Promise.all(cloudPromises)
                    .then(results => {
                        for (var i = 2; i >= 0; i--) {
                            ...
                            log('file uploaded is: ' + fileName);
                            dir = req.body.email;
                            fs.move('./uploads/' + fileName, './uploads/' + dir + '/' + newFileName + "/" + fileName, { overwrite: true }, err => {
...
router.post("/api/reviewUpload", function(req, res) {
    currentResult = res;
    currentRequest = req;
    ...
    uploadReview(req, res, function(err) {
        ...
        if (req.files['review_doc'] == undefined) {
            return res.end("Select a review file");
        }
        ...
        database.ref('clientFiles').child(key).once('value')
            .then(fileSnap => {
                ...
                sendFile = Object.keys(fileSnap.val())[0];
                var dir = gotFile.email;
                var newFileName = sendFile;

                var fileName = req.files['review_doc'][0].filename
                log('uploads/' + dir + '/' + newFileName + "/" + fileName);
                var cloudPromises = bucket.upload('./uploads/' + fileName, {
                    destination: 'uploads/' + dir + '/' + newFileName + "/reviewDoc"
                }, (err, file) => {
                    if (err) {
                        log(err);
                        return res.end("Error");
                    } else {
                        bucket.file('uploads/' + dir + '/' + newFileName + "/reviewDoc").acl.readers.addUser(sendTo);
                        sendEmail(sendTo, "Paper reviewed", "<a href=\"https://storage.cloud.google.com/raid-uploads/uploads/aditya.r.mhatre%40gmail.com/"+newFileName+"/reviewDoc\">Click here to see the review</a>")
                        return res.send("Uploaded<br><a href=\"/review\">Back</a>");
                    }
...
router.post('/api/download', (req, res, next) => {
    try {
        ...
        var paper_url = null;
        var gradeSheetManuscripts_url = null;
        var reviewerComments_url = null;
        ...
        database.ref('clientFiles').child(key).once('value')
            .then(fileSnap => {
                ...
                paper_url = url + "uploads/" + gotFile.email + "/" + sendFile.replace(/ /g, "%20") + "/" + gotFile.filename.replace(/ /g, "%20");
                gradeSheetManuscripts_url = url + "uploads/" + gotFile.email + "/" + sendFile.replace(/ /g, "%20") + "/" + gotFile.gradeSheetManuscripts.filename.replace(/ /g, "%20");
                reviewerComments_url = url + "uploads/" + gotFile.email + "/" + sendFile.replace(/ /g, "%20") + "/" + gotFile.reviewerComments.filename.replace(/ /g, "%20");
            })
            .then(() => {
                res.status(200).json({
                    status: "success",
                    paper_url: paper_url,
                    gradeSheetManuscripts_url: gradeSheetManuscripts_url,
                    reviewerComments_url: reviewerComments_url
                });
            });
    ...
    try {
        ...
        database.ref('clientFiles').child(key).once('value')
            .then(fileSnap => {
                ...
                return database.ref('acceptedFiles').child(key).child(sendFile).set(gotFile);
            })
            .then(() => {
...
router.post('/api/reupload', (req, res, next) => {
    log("Getting files for " + logged_in_user.email);
    var sendThis = { files: [] };
    database.ref('reviewers').once('value')
        .then(() => {
            database.ref('clientFiles').once('value').then(keySnapshot => {
                keySnapshot.forEach(fileSnapshot => {
                    fileSnapshot.forEach(file_s => {
                        var file = file_s.val();
                        if (file.email == logged_in_user.email)
                            sendThis.files.push({
                                email: file.email,
                                filename: file_s.key,
                                key: fileSnapshot.key
